...
st.title("🥊 UFC Fight Predictor (Polished Version)")
...
# Select the first two detected fighter columns
fighter_col1, fighter_col2 = fighter_columns[:2]

# Drop rows with missing fighter names
df = df.dropna(subset=[fighter_col1, fighter_col2])

# Identify numeric stats automatically (skip fighter names, winner, date, etc.)
...
for _, row in df.iterrows():
    for prefix, name in zip([fighter_col1, fighter_col2], [row[fighter_col1], row[fighter_col2]]):
        if name not in fighters:
            fighters[name] = []
        stats = {col: row[col] for col in stat_cols if pd.notnull(row[col])}
...
if fighter_A != fighter_B:
    ...
    comp_df = pd.DataFrame({
        "Stat": stat_cols,
        fighter_A: [A[s] for s in stat_cols],
        fighter_B: [B[s] for s in stat_cols]
    })
